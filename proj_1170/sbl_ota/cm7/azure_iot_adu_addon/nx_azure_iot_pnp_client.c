...
UINT nx_azure_iot_pnp_client_component_add_internal(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                                    ...
                                                    UINT (*callback_ptr)(NX_AZURE_IOT_JSON_READER *json_reader_ptr,
                                                                         ...
                                                                         VOID *args),
                                                    VOID *callback_args);
static UINT nx_azure_iot_pnp_client_desired_component_property_value_next_internal(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                                                                   NX_AZURE_IOT_JSON_READER *reader_ptr, UINT message_type,
                                                                                   const UCHAR **component_pptr, UINT *component_len_ptr,
                                                                                   UINT *component_index, UINT skip_system_component);
...
/* Convert all callbacks to generic callback and vice versa*/
typedef union NX_AZURE_IOT_PNP_CALLBACK
{
    NX_AZURE_IOT_HUB_TRANSPORT_GENERIC_FN nx_azure_iot_pnp_transport_cb;
    VOID (*nx_azure_iot_pnp_receive_cb)(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr, VOID *args);
    VOID (*nx_azure_iot_pnp_connection_status_cb)(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr, UINT status);
    VOID (*nx_azure_iot_pnp_receive_property_cb)(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                                 UINT request_id, UINT response_status,
                                                 ULONG version, VOID *arg);
} NX_AZURE_IOT_PNP_CALLBACK;
...
#ifndef NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE
static UINT nx_azure_iot_pnp_client_system_properties_process(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                                              NX_PACKET *packet_ptr,
                                                              ULONG topic_offset,
                                                              USHORT topic_length,
                                                              UINT message_type)
...
UCHAR *prepend_ptr = NX_NULL;
    ...
    else if (message_type == NX_AZURE_IOT_PNP_DESIRED_PROPERTIES)
    {

        /* Record the prepend pointer since nx_azure_iot_pnp_client_desired_properties_receive() will retreive the topic.  */
    ...
    while (nx_azure_iot_pnp_client_desired_component_property_value_next_internal(pnp_client_ptr,
                                                                                  &reader_ptr, message_type,
                                                                                  &component_ptr, &component_len,
                                                                                  &component_index, NX_FALSE) == NX_AZURE_IOT_SUCCESS)
    {

        /* Check if it is system component.  */
        if ((component_ptr) && (component_len) &&
            (pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback[component_index]))
        {
            status = pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback[component_index](&reader_ptr, version,
                                                                                                   pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback_args[component_index]);
            if (status)
            {
                return(status);
            }
    ...
    /* Recover the prepend pointer since nx_azure_iot_pnp_client_desired_properties_receive() will retreive the topic.  */
...
#ifndef NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE
        if ((status = nx_azure_iot_pnp_client_system_properties_process(pnp_client_ptr, packet_ptr, topic_offset, topic_length, message_type)))
        {
            return(status);
        }
#endif /* NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE */
...
{
NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr = (NX_AZURE_IOT_PNP_CLIENT *)hub_trans_ptr -> nx_azure_iot_hub_transport_client_context;
NX_AZURE_IOT_PNP_CALLBACK callback_union;

    callback_union.nx_azure_iot_pnp_transport_cb = arg1;
    callback_union.nx_azure_iot_pnp_receive_cb(pnp_client_ptr, arg2);
}
...
NX_AZURE_IOT_PNP_CALLBACK callback_union;
    ...
    callback_union.nx_azure_iot_pnp_transport_cb = arg;
    callback_union.nx_azure_iot_pnp_connection_status_cb(pnp_client_ptr, status);
}
...
NX_AZURE_IOT_PNP_CALLBACK callback_union;
    ...
    {
        LogError(LogLiteralArgs("IoTPnP failed to find reported property: %d"), status);
        return;
    }
    ...
    {
        LogError(LogLiteralArgs("IoTPnP parse twin failed: %d"), status);
        nx_packet_release(packet_ptr);
        return;
    }

    callback_union.nx_azure_iot_pnp_transport_cb = arg1;
    callback_union.nx_azure_iot_pnp_receive_property_cb(pnp_client_ptr, request_id,
                                                        response_status, version, arg2);
    nx_packet_release(packet_ptr);
}
...
#ifndef NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE
    pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback[length_of_componet_list] = callback_ptr;
    pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback_args[length_of_componet_list] = callback_args;
#endif /* NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE */
...
UINT nx_azure_iot_pnp_client_component_add(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                           ...
                                           UINT component_name_length)
{
    return(nx_azure_iot_pnp_client_component_add_internal(pnp_client_ptr, component_name_ptr, component_name_length, NX_NULL, NX_NULL));
}
...
NX_AZURE_IOT_PNP_CALLBACK callback_union;
    ...
    callback_union.nx_azure_iot_pnp_connection_status_cb = connection_status_cb;
    ...
    return(nx_azure_iot_hub_transport_connection_callback_set(&(pnp_client_ptr -> nx_azure_iot_pnp_client_transport),
                                                              ...
                                                              callback_union.nx_azure_iot_pnp_transport_cb));
}
...
NX_AZURE_IOT_PNP_CALLBACK callback_union;
    ...
    message_queue_index =  nx_azure_iot_pnp_client_mesg_type_to_queue_index(message_type);
    callback_union.nx_azure_iot_pnp_receive_cb = callback_ptr;
    ...
    return(nx_azure_iot_hub_transport_receive_message_callback_set(&(pnp_client_ptr ->nx_azure_iot_pnp_client_transport),
                                                                   ...
                                                                   callback_union.nx_azure_iot_pnp_transport_cb, callback_args));
}
...
NX_AZURE_IOT_PNP_CALLBACK callback_union;
    ...
    callback_union.nx_azure_iot_pnp_receive_property_cb = callback_ptr;
...
UINT nx_azure_iot_pnp_client_desired_component_property_value_next_internal(NX_AZURE_IOT_PNP_CLIENT *pnp_client_ptr,
                                                                            NX_AZURE_IOT_JSON_READER *reader_ptr, UINT message_type,
                                                                            const UCHAR **component_pptr, UINT *component_len_ptr,
                                                                            UINT *component_index, UINT skip_system_component)
{
...
UINT index;
    ...
    NX_PARAMETER_NOT_USED(component_index);
    NX_PARAMETER_NOT_USED(skip_system_component);
#endif /* NX_AZURE_IOT_PNP_CLIENT_SYSTEM_SERVICE_DISABLE */

    component_name = az_span_create((UCHAR *)component_pptr, (INT)*component_len_ptr);
    ...
    do
    {
        ...
        *component_len_ptr = (UINT)az_span_size(component_name);
        ...
        /* Check if it is root property.  */
        if ((*component_pptr == NX_NULL) || (*component_len_ptr == 0))
        {

            /* Yes, it is root property, return.  */
            return(NX_AZURE_IOT_SUCCESS);
        }

        /* Get the component index.  */
        index = 0;
        while (index < (UINT)pnp_client_ptr -> iot_pnp_client_core._internal.options.component_names_length)
        {
            if (az_span_is_content_equal(component_name, pnp_client_ptr -> nx_azure_iot_pnp_client_component_list[index]))
            {
                break;
            }
            index++;
        ...
        /* Check if skip system component.  */
        if (skip_system_component == NX_FALSE)
        {
            return(NX_AZURE_IOT_SUCCESS);
        }

        /* Check if it is system component.  */
        if (pnp_client_ptr -> nx_azure_iot_pnp_client_component_callback[index] != NX_NULL)
        {
...
UINT status;
    ...
    status = nx_azure_iot_pnp_client_desired_component_property_value_next_internal(pnp_client_ptr, reader_ptr, message_type,
                                                                                    component_pptr, component_len_ptr,
                                                                                    NX_NULL, NX_TRUE);
