...
void write_image_ok(void)
{
    ...
    primask = DisableGlobalIRQ();
    status = sfw_flash_erase(FLASH_AREA_IMAGE_1_OFFSET+FLASH_AREA_IMAGE_1_SIZE/2, 0x1000);
...
void enable_image(void)
{
    ...
    EnableGlobalIRQ(primask);
    PRINTF("write_image_ok SOC_REMAP_ENABLE=1 exit\n" ;
}
void write_image_ok(void)
{
    uint32_t off;
    ...
    EnableGlobalIRQ(primask);
    PRINTF("write_image_ok SOC_REMAP_ENABLE=0 exit\n" ;
}
#endif
