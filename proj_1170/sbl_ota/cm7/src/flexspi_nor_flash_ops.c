...
status_t sfw_flash_erase(uint32_t address, size_t len)
{
  PRINTF("erasing 0x%lx, 0x%lx \r\n", address, len);
	...
	for (; len > 0; len -= SECTOR_SIZE) {
		...
		if (status != kStatus_Success)
		{
		  PRINTF("sfw_flash_erase %lx, 0x%lx failed with %d\r\n", address, len*SECTOR_SIZE, status);
			return -1;
		}
	...
	return 0;
  PRINTF("erased %lx, 0x%lx \r\n", address, len*SECTOR_SIZE);
}

status_t sfw_flash_init(void)
{
  return 0;
  PRINTF("sfw_flash_init enter.\r\n");
	status_t status;
    
    /* Update LUT table. */
    FLEXSPI_UpdateLUT(EXAMPLE_FLEXSPI, 4, &customLUT[4], CUSTOM_LUT_LENGTH);
  PRINTF("FLEXSPI_UpdateLUT successful.\r\n");

    /* Do software reset. */
    FLEXSPI_SoftwareReset(EXAMPLE_FLEXSPI);
	
  PRINTF("FLEXSPI_SoftwareReset.\r\n");

	status = flexspi_nor_enable_quad_mode(EXAMPLE_FLEXSPI);

  PRINTF("sfw_flash_init exit.\r\n");
    return status;
}
